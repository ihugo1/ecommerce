...
export function createProductModal (productId){
    let productToShow = productsDataBase.find(product =>{ return product.id == productId; });
    ...
    `;
    showProductModal();
    document.querySelector(".modal__buy-btn").addEventListener("click", ()=>{ 
        addToShoppingCart(productToShow.id); 
    });
